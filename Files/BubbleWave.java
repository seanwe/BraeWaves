class BubbleWave { 
  float BubbleAngle = 120; //how far the bubbles extrude
  ...
  //constructor
  BubbleWave (int numSegments, float spawnRadius, float spawnOffset, Color lineColor, boolean fill) {  
  ...
  {
    float interiorAngle = (180 - BubbleAngle)/2;
  ...
  void drawShape() {
    ...
    //there are extra points at the begging and end to round it out right
    curveVertex(width/2 + points[numSegments*2-1].r * cos(points[numSegments*2-1].t), height/2 + points[numSegments*2-1].r * sin(points[numSegments*2-1].t));
    for (int i = 0; i < numSegments*2; i++)
    {
      cartX1 = width/2 + points[i].r * cos(points[i].t);
      cartY1 = height/2 + points[i].r * sin(points[i].t);
      curveVertex(cartX1, cartY1);
    }
    curveVertex(width/2 + points[0].r * cos(points[0].t), height/2 + points[0].r * sin(points[0].t));
    curveVertex(width/2 + points[1].r * cos(points[1].t), height/2 + points[1].r * sin(points[1].t));
